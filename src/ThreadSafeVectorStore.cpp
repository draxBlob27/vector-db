...
Result<std::vector<std::pair<std::uint64_t, float>>, DBError> ThreadSafeVectorStore::query(const Vector& q_vector, std::uint64_t k, Metric metric) const {//very large object is getting created, can think of move semantics
    ...
    std::shared_lock<std::shared_mutex> lk{entry_mutex}; //multiple readers allowed, shares lock across multiple reader threads.
    if (m_vectors.empty()) {
...
Result<Unit, DBError> ThreadSafeVectorStore::load(const std::string& filename) { //can use std::string_view -> improves perf, but does it matter that much. We'll seee
    ...
    std::vector<std::pair<uint64_t, Vector>> m_vectors_temp(count);

    for (std::uint64_t i = 0; i < count; i++)
    {
        m_vectors_temp[i].second.data.resize(dimension);

        inf.read(reinterpret_cast<char*>(&m_vectors_temp[i].first), sizeof(std::uint64_t));//reads and loads id.

        unsigned char *d_ptr = reinterpret_cast<unsigned char *>(&m_vectors_temp[i].second.data[0]);
        ...
        inf.read(reinterpret_cast<char *>(&m_vectors_temp[i].second.norm_data), sizeof(m_vectors_temp[i].second.norm_data));
        if (inf.fail() || inf.bad()) {
            return Err<DBError>{DBError::DataBaseEmptyError};
            // throw ArchiveError("Could not read file.");
        }
    ...
    std::swap(m_vectors, m_vectors_temp);
    return Ok<Unit>{Unit{}};
}