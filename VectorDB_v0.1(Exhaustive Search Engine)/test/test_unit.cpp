...
TEST_F(VectorStore_test, save_load_trip) {
    ...
    auto res_save{vdb.save("/home/sp27022003/vector-db/VectorDB_v0.1(Exhaustive Search Engine)/dump/save_load_trip.bin")};
    ASSERT_TRUE(res_save.is_ok()) << res_save.err_value();
    vdb = VectorStore{};
    
    auto res_load{vdb.load("/home/sp27022003/vector-db/VectorDB_v0.1(Exhaustive Search Engine)/dump/save_load_trip.bin")};
