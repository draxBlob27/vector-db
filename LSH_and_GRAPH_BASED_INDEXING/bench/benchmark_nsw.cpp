...
int main() {
    ...
    auto calc_qps{[&](const NSW_Index& nsw) {
        ...
        std::ofstream outf{"/home/sp27022003/vector-db/LSH_and_GRAPH_BASED_INDEXING/bench/benchmark_nsw.txt"};

        for (std::size_t i{0}; i < num_queries; i++) {
            auto res = nsw.query(queries[i], my_k, 120); //impilcit conversion fo float query to Vector query
