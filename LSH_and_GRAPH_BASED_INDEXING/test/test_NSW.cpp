...
TEST_F(NSW_test, save_test) {
    ASSERT_NO_THROW(nsw.save("/home/sp27022003/vector-db/LSH_and_GRAPH_BASED_INDEXING/persist/nsw_" + std::to_string(NSW_Index::getIndexNumber()) + ".bin"));
}

TEST_F(NSW_test, load_test) {
    NSW_Index check_load{};
    ASSERT_NO_THROW(check_load.load("/home/sp27022003/vector-db/LSH_and_GRAPH_BASED_INDEXING/persist/nsw_" + std::to_string(NSW_Index::getIndexNumber()) + ".bin"));
